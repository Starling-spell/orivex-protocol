...
document.querySelectorAll('[data-view="register"]').forEach(el=>el.addEventListener('click',openRegister));
closeModal?.addEventListener('click',()=>{if(modal)modal.hidden=true});
modal?.addEventListener('click',e=>{if(e.target===modal)modal.hidden=true});
if(modal&&location.hash==='#register-agent')modal.hidden=false;
...
if(verifyBtn){verifyBtn.textContent='Verification preview';verifyBtn.addEventListener('click',()=>toast('Demo certificate only. GenLayer verification is not connected.'));}
let lastFocused;
document.querySelectorAll('[data-view="register"]').forEach(el=>el.addEventListener('click',()=>{lastFocused=el;closeModal?.focus()}));
