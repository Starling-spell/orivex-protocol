...
describe('proof verification boundary', () => {
  it('reproduces the actual Studio Next proof commitment', () => {
  ...
  it.each([{ chain_id: 61999 }, { chain_id: 1 }, { proof_id: 2 }, { status: 'PENDING' }, { domain: 'other' }])('rejects a changed domain or unfinalized proof: %o', changed => {
  ...
  it('rejects a finalized execution error and leader-only execution', () => {
    const receipt = { statusName: 'FINALIZED', txExecutionResultName: 'FINISHED_WITH_RETURN', status: 7, result_name: 'MAJORITY_AGREE', leader_only: false, to_address: manifest.address,
